...
The observations below were collected before arithmetic retrieval and deep file
discovery were integrated. They are historical model-only measurements, not new
latency or quality measurements of the combined application. No inference was run
for this refresh. The model matrix now removes MCP servers and uses a tool-denied
benchmark agent, including when reopening an older prepared matrix configuration.
Its synthetic evidence cannot cause the copied product config to scan real files.
