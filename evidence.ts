...
export function wantsWeb(query:string){
 ...
 if(/^\d+(?:\.\d+)?\s+(?:USD|EUR|GBP|JPY|CAD|AUD|CHF|CNY)\s+(?:to|in)\s+(?:USD|EUR|GBP|JPY|CAD|AUD|CHF|CNY)[?]?$/i.test(q))return true
 if(/^(?:calculate|compute)\b.*\b(?:latest|current)\b/i.test(q)||/^\d.*\b(?:current|latest)\b.*\b(?:price|rate|stock)\b/i.test(q))return true
