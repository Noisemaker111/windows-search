...
const config=await Bun.file(join(import.meta.dir,'runtime-config','opencode.json')).json()
...
for(const q of queries){
 ...
 if(!hits.length&&!/^(where|find|locate|open|play|run|launch)\b/i.test(q.query))try{
